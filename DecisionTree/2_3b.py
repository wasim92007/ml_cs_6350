...
if __name__ == '__main__':
    ...
    '''
    def get_majority_labels_unkown(labels):
        print(f'labels:{labels}')
        mod_labels = list(filter(lambda x:x != "unknown", labels))
        print(f'mod_labels:{mod_labels}')
        return stats.mode(mod_labels)[0][0]

    labels = ["unknown","unknown","unknown",'a','a','b']
    mode_label = get_majority_labels_unkown(labels)
    print(mode_label)
    '''

    ## List of column that has unkown value
    unknown_col_list = [1, 3, 8, 15]

    ## Get the most common value for the unknown
    majority_attrib_val_dic = {}
    for attrib, col in attrib_name_col_dic.items():
        if col in unknown_col_list:
            majority_attrib_val = get_majority_label(ori_test_ds[:,col], filter_unknown=True) 
            majority_attrib_val_dic[col] = majority_attrib_val
    #print(f'majority_attrib_val:{majority_attrib_val_dic}')


    ## Convert unkown to majority
    train_ds = replace_unknown_with_major(dataset=ori_train_ds, unknown_col_list=unknown_col_list, majority_attrib_val_dic=majority_attrib_val_dic)
    test_ds = replace_unknown_with_major(dataset=ori_test_ds, unknown_col_list=unknown_col_list, majority_attrib_val_dic=majority_attrib_val_dic)
    #print(ori_test_ds[:5])
    #print(test_ds[:5])
