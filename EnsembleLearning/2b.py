...
if __name__ == '__main__':
    ...
    ## Perform unit test
    train_prediction, train_prediction_hist = bagging_cs.predict_datapoint(datapoint=train_ds[2], pos_label='yes', neg_label='no')
    #print(f'train_prediction:{train_prediction}')
    #print(f'train_prediction_hist:{train_prediction_hist}')

    ## Perform whole train_ds test
    train_predictions, train_predictions_hist = bagging_cs.predict_dataset(dataset=train_ds, pos_label='yes', neg_label='no')
    #print(f'train_predictions[:5]:{predictions[:5]}')
    #print(f'train_predictions_hist[:5]:{train_predictions_hist[:5]}')

    train_predictions_hist_dic = {t:[] for t in range(bagging_cs.num_rounds)}
    for train_pred_datapoint_hist in train_predictions_hist:
        for t in range(bagging_cs.num_rounds):
    ...
    for t in range(bagging_cs.num_rounds):
        ...
        bagging_cs.classifiers[t]['ada_eta_t_train'] = ada_eta_t_train
        #print(f'eta_t_train:{bagging_cs.classifiers[t]["eta_t_train"]}')
        #print(f'ada_eta_t_train:{bagging_cs.classifiers[t]["ada_eta_t_train"]}')

    ## Perform whole test_ds test
    test_predictions, test_predictions_hist = bagging_cs.predict_dataset(dataset=test_ds, pos_label='yes', neg_label='no')
    #print(f'test_predictions[:5]:{predictions[:5]}')
    #print(f'test_predictions_hist[:5]:{test_predictions_hist[:5]}')

    test_predictions_hist_dic = {t:[] for t in range(bagging_cs.num_rounds)}
    ...
    for t in range(bagging_cs.num_rounds):
        ...
        bagging_cs.classifiers[t]['ada_eta_t_test'] = ada_eta_t_test
        #print(f'eta_t_test:{bagging_cs.classifiers[t]["eta_t_test"]}')
        #print(f'ada_eta_t_test:{bagging_cs.classifiers[t]["ada_eta_t_test"]}')
    ...
    ada_test_errors  = []
    for t in range(bagging_cs.num_rounds):
        #alpha_ts.append(bagging_cs.classifiers[t]['alpha_t'])
        classifer_train_errors.append(bagging_cs.classifiers[t]['eta_t_train'])
        classifer_test_errors.append (bagging_cs.classifiers[t]['eta_t_test'])
        ada_train_errors.append(bagging_cs.classifiers[t]['ada_eta_t_train'])
        ada_test_errors.append (bagging_cs.classifiers[t]['ada_eta_t_test'])
    ...
    print(f'Plotting bagging decision tree error vs t graph')
    label_dic = {'xlabel': 'Iteration(t)', 'ylabel': 'Error(decision tree)', 'legend':['train', 'test'], 'title':'Bagging full random decision tree error'}
    plot_two_data_func(data_1=classifer_train_errors, data_2=classifer_test_errors, graph_name='./results/2b_classifier_error.png', label_dic=label_dic)

    ## Plot the ensemble train test error variation with t
    print(f'Plotting bagging ensemble error vs t graph')
    label_dic = {'xlabel': 'Iteration(t)', 'ylabel': 'Error(Bagging Ensemble)', 'legend':['train', 'test'], 'title':'Bagging ensemble error'}
    plot_two_data_func(data_1=ada_train_errors, data_2=ada_test_errors, graph_name='./results/2b_ensemble_error.png', label_dic=label_dic)