...
if __name__ == '__main__':
    ...
    ## Iterate over the number of Bagged Tree to save memory
    ...
    for n in range(num_bagged_tree):
        ...
        random_forest_cs = Bagging(max_depth=max_depth,
        ...
        ## Perform bagging
        random_forest_cs.perform_bagging(train_ds=train_ds[:],
                                   test_ds=test_ds[:],
    ...
    ## Average prediction
    single_pred_bin_avg = np.sum(single_pred_list, axis=1)/num_bagged_tree
    random_forest_pred_bin_avg = np.sum(random_forest_pred_list, axis=1)/num_bagged_tree

    #### Calculate bias
    ## For single random tree
    single_pred_bias = np.average((gt_bin - single_pred_bin_avg) ** 2)
    single_pred_var  = np.array([(gt_bin - single_pred_list[:,i]) ** 2 for i in range(single_pred_list.shape[1])]).sum(axis=1)/num_bagged_tree
    single_pred_var = np.average(single_pred_var)
    single_pred_gse  = single_pred_bias + single_pred_var

    print(f'First decision tree bias: {single_pred_bias}')
    print(f'First decision tree variance: {single_pred_var}')
    print(f'First decision tree gse: {single_pred_gse}')


    ## For Random Forest
    random_forest_pred_bias = np.average((gt_bin - random_forest_pred_bin_avg) ** 2)
    random_forest_pred_var  = np.array([(gt_bin - random_forest_pred_list[:,i]) ** 2 for i in range(random_forest_pred_list.shape[1])]).sum(axis=1)/num_bagged_tree
    ...
    print(f'Bagged Tree bias: {random_forest_pred_bias}')
    print(f'Bagged Tree variance: {random_forest_pred_var}')
    print(f'Bagged Tree gse: {random_forest_pred_gse}')
