...
class Decision_Tree(object):
    ...
    def get_feat_subset(self, attrib_name_vals_dic, num_max_feat_subset):
        num_feat = len(attrib_name_vals_dic)
        if num_feat < num_max_feat_subset:
            return attrib_name_vals_dic
        else:
            random_list = [random.randint(0, num_feat-1) for i in range(num_max_feat_subset)]
            mod_attrib_name_vals_dic = {}
            for i, (attrib_name, attrib_vals) in enumerate(attrib_name_vals_dic.items()):
                if i in random_list:
                    mod_attrib_name_vals_dic[attrib_name] = attrib_vals
        return mod_attrib_name_vals_dic


    def id3_algorithm(self, dataset, attrib_name_col_dic, attrib_name_vals_dic, depth=0, filter_unknown=False, *args, **kwargs):
        '''
        ...
        if dt_root_node.depth >= self.max_depth:
            ...
            majority_label = get_majority_label(labels=labels, filter_unknown=filter_unknown, weights=weights)
        ...
        random_tree = kwargs.get('random_tree', False)
        #print(f'random_tree:{random_tree}')
        if random_tree:
            #print(f'random_tree is passed')
            num_max_feat_subset = kwargs.get('num_max_feat_subset', 2)
            mod_attrib_name_vals_dic = self.get_feat_subset(attrib_name_vals_dic=attrib_name_vals_dic, num_max_feat_subset=num_max_feat_subset)
        else:
            mod_attrib_name_vals_dic = attrib_name_vals_dic
        ## Get the attribute to split on
        max_attrib, max_gain = get_attribute_for_max_info_gain(dataset=dataset, attrib_name_vals_dic=mod_attrib_name_vals_dic, attrib_name_col_dic=attrib_name_col_dic, return_pair=True, entropy_func=self.entropy_func, weights=weights) 
        ## Initialize sub dataset
        sub_dataset = {}
        ## Intialize sub weights
        sub_weights = {}
        ...
        for val in attrib_vals:
            ...
            if numeric_case:
                sub_dataset[val], sub_weights[val] = split_based_on_attrib(attrib=max_attrib, val=self.num_def_val, attrib_name_col_dic=attrib_name_col_dic, dataset=dataset, weights=weights, return_dataset=True, num_def_val=self.num_def_val, first_half=val=='left')
            else:
                sub_dataset[val], sub_weights[val] = split_based_on_attrib(attrib=max_attrib, val=val, attrib_name_col_dic=attrib_name_col_dic, dataset=dataset, weights=weights, return_dataset=True)
            
            child_node = Tree_Node()
            ## If we have some datapoint in the splitted dataset on the attrib value
            if len(sub_dataset[val]) > 0:
                child_node = self.id3_algorithm(dataset=sub_dataset[val], attrib_name_col_dic=attrib_name_col_dic, attrib_name_vals_dic=new_attrib_name_vals_dic, depth=depth+1, weights=sub_weights[val], random_tree=random_tree, num_max_feat_subset=num_max_feat_subset)
            ...
            ## If we do not have any datapoint in the splitted dataset on the attrib value
                ...
                majority_label = get_majority_label(dataset[:,-1], filter_unknown=filter_unknown, weights=weights)
            ...
            if numeric_case:
                ...
                median_val = median_val = median(map(float,dataset[:,attrib_name_col_dic[max_attrib]]))
    ...
    def build_tree(self, dataset, attrib_name_col_dic, attrib_name_vals_dic, max_depth=None, entropy_func=None, *args, **kwargs):
        '''
        ...
        #self.dtroot = self.id3_algorithm(dataset=dataset, attrib_name_col_dic=attrib_name_col_dic, attrib_name_vals_dic=attrib_name_vals_dic, depth=0, weights=kwargs.get('weights', None), args=args, kwargs=kwargs)
        self.dtroot = self.id3_algorithm(dataset=dataset, attrib_name_col_dic=attrib_name_col_dic, attrib_name_vals_dic=attrib_name_vals_dic, depth=0, weights=kwargs.get('weights', None), random_tree=kwargs.get('random_tree', False))
            ...
            for child_node in dtroot.children:
                if child_node:
                    ...
                    if node_attrib_col in self.numeric_col_list:
                        ...
                        if float(datapoint[node_attrib_col]) <= child_node.node_median_val and node_val == 'left':
                            dtroot = child_node
                            break
                        #if int(datapoint[node_attrib_col]) > child_node.node_median_val and node_val == 'right':
                        if float(datapoint[node_attrib_col]) > child_node.node_median_val and node_val == 'right':
                            dtroot = child_node
                            break
                    else:
                        if datapoint[node_attrib_col] == node_val:
