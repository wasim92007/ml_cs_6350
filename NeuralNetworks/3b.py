...
if __name__ == '__main__':
    ...
    for width in widths:
        ...
        model = nn.NN_Sequential([
            nn.FC_Layer(in_channels=4, out_channels=width, act_func='sigmoid', initialization='normal'),
            nn.FC_Layer(in_channels=width, out_channels=width, act_func='sigmoid', initialization='normal'),
            nn.FC_Layer(in_channels=width, out_channels=width, act_func='sigmoid', initialization='normal'),
            nn.FC_Layer(in_channels=width, out_channels=1, act_func='linear', initialization='normal', include_bias=False),
        ])
        ...
        plt.savefig(f'./results/homemade_nn_act_func_sigmoid_depth_2_width_{width}.png')
        plt.close()
